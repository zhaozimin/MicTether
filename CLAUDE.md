...
build_dmg.sh - 一键 archive + Developer ID 签名(硬化+时间戳+沙盒,自动选钥匙串身份,缺证书降级 ad-hoc) + 打 DMG(容器亦签名) + 检测到 notarytool 凭据(profile=MicTether-notary)则自动公证装订；产物恒名 `MicTether-macOS.dmg`,供网站 latest 下载链 `releases/latest/download/MicTether-macOS.dmg` 恒定指向
